...
const TreeCanvas = (
  ...
  const drawTree = useCallback(
    (height: number, width: number, dx: number, dy: number) => {
      ...
      const recursiveDraw = (
        ...
        for (const c of node.children) {
          ...
          ctx.lineTo(
            Math.floor(cumulative * widthChunk + (w * widthChunk) / 2 + start),
            500 + level * 400
          )
          ctx.lineWidth = 6
          ctx.lineCap = 'round'
  ...
  return (
    <Canvas
      ...
      onTouchStart={onTouchDownCanvas}
      onTouchMove={onTouchMoveCanvas}
      onTouchEnd={onTouchUpCanvas}
    />
  )
}

export default forwardRef<HTMLCanvasElement, TreeCanvasProps>(TreeCanvas)
